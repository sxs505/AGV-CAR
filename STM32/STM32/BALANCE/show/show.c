...
void show_task(void *pvParameters)
{
   ...
   {	
		...
		else if(10<=Voltage && Voltage<10.5 && LowVoltage_1<2)LowVoltage_1++; //10.5V低电量时蜂鸣器第一次报警
		else if(Voltage<10 && LowVoltage_1<2)LowVoltage_2++; //10V小车禁止控制时蜂鸣器第二次报警
...
{    
	 ...
	 printf("{A%d:%d:%d:%d}$",(u8)Left_Figure,(u8)Right_Figure,Voltage_Show,(int)gyro[2]); //打印APP数据到APP上面
	 else
