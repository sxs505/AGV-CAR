...
{ 
    ...
    {	
			...
			if(Check==0)
			{
				...
				if(Turn_Off(Voltage)==0)   //如果电池电压不存在异常, 使能开关ON, 软件失能标志位为0
				 { 			 	
...
void Get_RC(void)
{
	...
	if(Car_Mode==Mec_Car||Car_Mode==Omni_Car)
	{
	 ...
	 {	
				if     (Flag_Left ==1)  Move_Z= PI/2*(RC_Velocity/500); //左自旋   
				else if(Flag_Right==1)  Move_Z=-PI/2*(RC_Velocity/500); //右自旋		
				else 		                Move_Z=0;                       //停止
	 }
	}	
	else
	{
	 ...
	 if     (Flag_Left ==1)  Move_Z= PI/2; //左自旋   
	 else if(Flag_Right==1)  Move_Z=-PI/2; //右自旋		
...
void PS2_control(void)
{
	  ...
	  if(Car_Mode==Mec_Car||Car_Mode==Omni_Car)
		...
		Move_X=Move_X/1000;        //单位转换 mm/s->m/s
		Move_Y=Move_Y/1000;        //单位转换 mm/s->m/s
		Move_Z=Move_Z;
...
void Remote_Control(void)
{
    ...
    LX=Remoter_Ch2-1500; //左摇杆前后方向 前进后退
    LY=Remoter_Ch4-1500; //左摇杆左右方向 左右移动 麦轮全向轮才使用到改通道，同时阿克曼小车该通道用做PWM输出控制舵机
	  RX=Remoter_Ch3-1500; //右摇杆前后方向 油门/加减速
    RY=Remoter_Ch1-1500; //右摇杆左右方向 自转
